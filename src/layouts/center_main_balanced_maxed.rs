...
pub fn update(workspace: &Workspace, windows: &mut Vec<&mut Window>, tags: &mut Vec<Tag>) {
    ...
    /*let primary_width = match window_count {
        ...
        2 => ((workspace.width() as f32 / 2.0) / 100.0 * workspace.main_width(tags)).floor() as i32,
        _ => ((workspace.width() as f32 / 3.0) / 100.0 * workspace.main_width(tags)).floor() as i32,
    };*/
    let primary_width = ((workspace.width() as f32 / 100.0) * workspace.main_width(tags)).floor() as i32;
